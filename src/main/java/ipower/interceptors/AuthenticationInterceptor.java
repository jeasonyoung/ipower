...
public class AuthenticationInterceptor extends AbstractInterceptor {
	...
	public String intercept(ActionInvocation invocation) throws Exception {
		...
		UserIdentity identity = (UserIdentity)session.get(BaseAction.CURRENT_USER_SESSION_KEY);
		if(identity == null){
			logger.info("未获取身份信息，跳转至登录Action：" + BaseAction.LOGIN);
			return BaseAction.LOGIN;
		}
		logger.info("身份验证成功，准备注入用户信息[" + identity.getId()+ "]"+ identity.getName());
		IUserAware userAware = (IUserAware)invocation.getAction();
		if(userAware != null){
			logger.info("注入用户信息成功！");
			userAware.setUserIdentity(identity);
		}
		logger.info("完成身份认证。");
		return invocation.invoke();
	}
